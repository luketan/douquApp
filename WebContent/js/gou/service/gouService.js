...
angular.module('ionicApp.gouServices', [])
			.service('_gouStatus',function($rootScope){
				$rootScope.goodsStatus = [{name:'--请选择--',value:''},{name:'上线',value:1},{name:'已售完',value:2},{name:'已下架',value:3},{name:'删除',value:4},{name:'待审核',value:5}];
			})
			.service('_gouBrand',function($rootScope,_url,_ajax){
			...
			.service('_gouActivity',function($rootScope,_url,_ajax){
				this.list = function($scope){
					_ajax.ajax({
...
//								return result.result;
			...
			.service('_gouType',function($rootScope,_url,_ajax){
				this.list = function($scope){
					_ajax.ajax({
			...
			.service('_gouStyle',function($rootScope,_url,_ajax){
			...
			.service('_gouFormat',function($rootScope,_url,_ajax){
				this.list = function($scope,req){
					_ajax.ajax({
